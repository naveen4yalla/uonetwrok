...
App={
    loading:false,
    ipfs: window.IpfsApi('infura-ipfs.io', 5001,{ protocol: 'https'},{ commands: ['id', 'dag', 'version'] }),
    ...
    loadContract: async () => {
        ...
        else if (App.network =="private")App.contracts.dwitter = new web3.eth.Contract(abi, mx);
            ...
            $('#registerBtn').on("click",async ()=>{
              ...
              reader1.onloadend = async function() {
                 ...
                 // var result1=await App.ipfs.files.add(buf1) // Upload buffer to IPFS
                  console.log("adsfasddsag")
                 // var result2=await  App.ipfs.files.add(buf2);
                 // console.log(result1[0].hash);
                 // console.log(result2[0].hash);
                  console.log("sakgdhfkas")
                  await App.contracts.dwitter.methods.registerUser($("#username").val(),$("#name").val(),"","",$("#bio").val()).send({from:App.account});
 ...
 //$("#userProfileImage").css("background-image", "url( https://ipfs.io/ipfs/" + App.user.imghash+ ")");;
  ...
  $("#userProfileImage").css("url('../images/batman.jpg')");;
  $("#userCoverImage").css("url('../images/clock.jpg)");;
        }
      },
  ...
  dweet:async()=>{
    ...
    if(image){
      ...
      reader1.onloadend = async function() {
        ...
        // hash=result[0].hash;
         await App.contracts.dwitter.methods.createDweet($("#dweetTag").val(),$("#dweetContent").val(),"").send({from:App.account});
         $("#dweetModalMsg").text("posted!!!");
      }
    }else{
      await App.contracts.dwitter.methods.createDweet($("#dweetTag").val(),$("#dweetContent").val(),"").send({from:App.account});
      $("#dweetModalMsg").text("posted!!!");
